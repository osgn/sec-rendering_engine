			...
			using(Visio2003Adapter va = new Visio2003Adapter(ruleFilesFolder + "test-3_0" + (strict?"-strict":"") + ".vdx",
				...
				foreach(Implication i in va.Implications) {
					if (i.Label == "imp1") {
						...
						Assert.AreEqual(typeof(byte[]), atom.GetPredicateValue(1), "Typed predicate support");

						Assert.AreEqual(typeof(Function), atom.Members[2], "Basic slot type predicate support");
						Assert.AreEqual("Size", atom.SlotNames[2], "Basic slot name predicate support");

						Assert.AreEqual(typeof(int), atom.GetPredicateValue(3), "Typed slot type predicate support");
